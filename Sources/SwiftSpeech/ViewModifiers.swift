    ...
    struct ToggleRecordingOnTap: ViewModifier {
        ...
        fileprivate func startRecording() {
            ...
            AudioServicesPlaySystemSoundWithCompletion(1117) {
                let session = SwiftSpeech.Session(id: SpeechRecognizer.ID(), configuration: sessionConfiguration)
                recordingSession = session
                delegate.onStartRecording(session: session)
                session.startRecording()
            }
...
public extension SwiftSpeech.ViewModifiers {
    struct OnRecognize: ViewModifier {
        ...
        {
            _model = State(initialValue: Model(isPartialResultIncluded: isPartialResultIncluded, switchToLatest: switchToLatest, resultHandler: resultHandler, errorHandler: errorHandler))
        }
